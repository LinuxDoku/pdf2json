...
(function () {
    ...
    function copyState(o1, o2) {
        ...
        o2.clippingPath = o1.clippingPath.slice();
    }
    ...
    let _drawPDFLine = function(p1, p2, lineWidth, color) {
        for (var clip of this.clippingPath) {            
            if (p1.x < clip.x1) {
                p1.x = clip.x1;
            }
            if (p1.y < clip.y1) {
...
//        let opacity = a.alpha * this.globalAlpha;
    ...
    contextPrototype.clip = function () {
        if (this.currentPath_.length < 4) {
            return;
        }

        // only rectangles are supported
        this.clippingPath.push({
            x1: this.currentPath_[3].x,
            y1: this.currentPath_[3].y,

            x2: this.currentPath_[1].x,
            y2: this.currentPath_[1].y
        });
    };
